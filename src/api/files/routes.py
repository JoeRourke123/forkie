...
signinBP = Blueprint('file_query', __name__, template_folder='../../templates', static_folder='../../static', url_prefix='/api')
query_schema = {
    "type": "object",
    "properties": {
...
@signinBP.route("/file_query", methods=["POST"])
def file_query():
    ...
    try:
        ...
        try:
            ...
            # all_rows = query.all()
            query = FileTable.query.order_by(FileTable.filename)
            if len(data.keys()) > 0:
                if "filename" in data:
