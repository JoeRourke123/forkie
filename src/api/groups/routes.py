...
def removeMember():
    ...
    if request.cookies.get("userid"):
        try:
            ...
            if not group or (group.groupleaderid is not request.cookies.get("userid") and request.cookies.get("userid") is not data["userid"]):
                return redirect(url_for('dash', msg="Sorry you are not permitted to complete this action"))

            UserGroupTable.delete().where(and_(userid=data["userid"], groupid=data["groupid"]))
            UserGroupTable.execute()
